...
import com.example.artificial_intelegent.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    val range_Sedang = Array<Int>(5){it*5+45}
    val range_Buruk = Array<Int>(5){it*5+66}
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnKalkulasi.setOnClickListener {
            if(binding.etvisko.text.toString().isEmpty() || binding.etvisko.text.toString().isEmpty()){
                Toast.makeText(this, "Data tidak boleh kosong", Toast.LENGTH_SHORT).show()
            }else{
                visko = binding.etvisko.text.toString().toDouble()
                densi = binding.etdensi.text.toString().toDouble()
                Fuzzification()
                Inference()
                Defuzzification()
                startActivity(Intent(this, output::class.java).also {
    ...
    fun disjunction(arr : Array<String>){
        ...
        // misal nama yang beda pada aray.. maka sipisah + di samakan (Baik,buruk, buruk, sedang)
        for(j in tempo2){
            val a = tempo2[0]
        ...
        if(tempo5.isNotEmpty()){
            ...
            //njukut nilai tertinggi
            tempo3 += result.toList().sortedDescending().first()
            result = arrayOf()
            // ambil nilai string dari nilai index
            if(tempo[0] > tempo[1]){
                val a = tempo[0]
                DK_disjuction += DK_inference[a]
            }else{
...
//                DK_disjuction += DK_inference[i.toInt()]
    ...
    private fun Defuzzification(){
            ...
            } else if (DK_disjuction.size == 1) {
                if (DK_disjuction[0] == "Buruk") {
                    final_result =
                        (range_Buruk.sum() * number_disjuction[0]) / (number_disjuction[0] * 5)
                } else if (DK_disjuction[0] == "Sedang") {
                    final_result =
                        (range_Sedang.sum() * number_disjuction[0]) / (number_disjuction[0] * 5)
                } else if (DK_disjuction[0] == "Bagus") {
                    final_result =
                        (range_Bagus.sum() * number_disjuction[0]) / (number_disjuction[0] * 5)
                } else {
                    println("Terjadi Kesalahan 4")
                }
            }else{
                Toast.makeText(this, "data tidak valid 5", Toast.LENGTH_SHORT).show()
            }
